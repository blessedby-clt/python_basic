...
def is_validated_number(user_input_number):
    ...
    if is_digit(user_input_number) == True & is_between_100_and_999(user_input_number) == True :
        three_digit = str(user_input_number)
        if is_duplicated_number(three_digit) == False :
            result = True
        else:
            result = False
    else :
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    user_list = [int(a) for a in str(user_input_number)]
    random_list = [int(a) for a in str(random_number)]
