...
def is_validated_morse_code(user_input):
    ...
    for idx in user_input.split(" ") :
        if idx in get_morse_code_dict().values() :
            count += 1
        else :
            error += 1
...
def decoding_character(morse_character):
    ...
    result_list = [i for i, j in morse_code_dict.items() if j == morse_character]
...
def encoding_character(english_character):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    morse_code_dict = get_morse_code_dict()
    result_list = [j for i, j in morse_code_dict.items() if i == english_character]
    result = result_list[0]
...
def decoding_sentence(morse_sentence):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    result_list = [[decoding_character(idx) for idx in element.split(" ")] for element in morse_sentence.split("  ")]
    result = " ".join(["".join(i) for i in result_list])
...
def encoding_sentence(english_sentence):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    english_sentence = ' '.join(get_cleaned_english_sentence(english_sentence).upper().split())
    result_list = [[encoding_character(idx) for idx in element] for element in english_sentence.split(" ")]
    result = "  ".join([" ".join(i) for i in result_list])
...
def main():
    ...
    while user_input != "0" :
        ...
        if is_help_command(user_input) == True :
            ...
            break
        elif is_validated_morse_code(user_input) == False | is_validated_english_sentence(user_input) == False :
            print("Wrong Input")
            continue
        else :
